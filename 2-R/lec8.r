...
The first script
We first used the new “dressed-up” data in dataForAnalysis.RData to repeat the analysis from Lecture 6.
However, we went to great length to always keep nice variable names and column and row names, wheverever
needed, such that we will get nice labels and titles in figures, tables, and titles.
...
The second script
In the second script, we wrap all of the previous script into a function. Or, I should say, we wrap almost all
of the previous script into a function. We no longer need the parameters at the beginning of the previous
script. These become the arguments of the function. See the very end of the below script.
##############################################
...
A fourth script
In our fourth script we call the function getAnalysis() to use it inside a loop and produce a large number
of graphs.
rm(list = ls()) # Empty workspace to start with a "clean sheet"
# REPLACE THE WORKING DIRECTORY BELOW WITH THE ONE FOR YOUR DEVICE
#setwd("D:/Dropbox/Mac&Surf/Programmierkurs Dropb/Data")
